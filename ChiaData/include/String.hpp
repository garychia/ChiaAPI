...
{
  private:
    template <class Char> static size_t CalculateRequiredSpace(const Char *cStr)
    {
        size_t strLength = 0;
        size_t i = 0;
        while (cStr[i])
    ...
    template <class Char> static size_t CalculateRequiredSpace(const Char *cStr, size_t size)
    {
        size_t strLength = 0;
  ...
  public:
    ...
    template <class T>
    String(const T *str, size_t length) noexcept : Str<CharType>('\0', CalculateRequiredSpace<T>(str, length))
    {
        size_t currentIdx = 0;
    ...
    template <class Char> String(const Char *cStr) : Str<char16_t>('\0', CalculateRequiredSpace<Char>(cStr))
    {
        size_t currentIdx = 0;
        size_t i = 0;
        while (cStr[i])
